import type { Block, Document, InlineMark, MarkCoverage, SelectionRange, TextSpan } from './marks.types';
...
const addMark = (marks: InlineMark[], mark: InlineMark): InlineMark[] => {
    ...
    const filteredMarks = marks.filter(
        (currentMark) => currentMark === 'bold' || currentMark === 'italic' || currentMark.type !== 'link',
    );
...
export const getSelectionMarks = (documentModel: Document, selectionRange: SelectionRange): InlineMark[] => {
    const normalizedSelection = normalizeSelectionRange(documentModel, selectionRange);
...
export const deleteSelectionRange = (
    documentModel: Document,
    selectionRange: SelectionRange,
): { document: Document; selection: SelectionRange } => replaceSelectionWithText(documentModel, selectionRange, '', []);
