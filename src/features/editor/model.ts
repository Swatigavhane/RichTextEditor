...
const marksAreEqual = (left: InlineMark[], right: InlineMark[]): boolean =>
    left.length === right.length && left.every((mark, index) => markKey(mark) === markKey(right[index]));
...
export const normalizeDocument = (documentModel: DocumentModel): DocumentModel => ({
    blocks: documentModel.blocks.length > 0
...
export const selectionPointToLinearOffset = (
    ...
    return getBlockStartOffset(documentModel, blockIndex) + clampOffset(normalizedPoint.offset, 0, getBlockTextLength(block));
};
...
export const linearOffsetToSelectionPoint = (
    ...
    const lastBlock = documentModel.blocks[documentModel.blocks.length - 1] ?? createParagraph('block-1');
...
export const linearRangeToSelectionRange = (
    ...
    const anchor = linearOffsetToSelectionPoint(documentModel, linearRange.isBackward ? linearRange.end : linearRange.start);
    const focus = linearOffsetToSelectionPoint(documentModel, linearRange.isBackward ? linearRange.start : linearRange.end);
...
const addMark = (marks: InlineMark[], mark: InlineMark): InlineMark[] => {
    ...
    const filteredMarks = marks.filter((currentMark) => currentMark === 'bold' || currentMark === 'italic' || currentMark.type !== 'link');
...
const removeMark = (marks: InlineMark[], mark: InlineMark): InlineMark[] => {
    if (typeof mark === 'string') {
        return marks.filter((currentMark) => currentMark !== mark).map((currentMark) => (typeof currentMark === 'string' ? currentMark : { ...currentMark }));
    }
...
export const toggleInlineMark = (
    ...
    const mode = getMarkCoverage(normalizedDocument, normalizedSelection, mark) === 'present' ? 'remove' : 'add';
...
export const applyInlineMark = (
    ...
    return normalizeDocument({
        blocks: normalizedDocument.blocks.map((block, blockIndex) => {
            ...
            return transformBlockRange(block, overlapStart - blockStart, overlapEnd - blockStart, mark, 'add');
        }),
    });
};
...
export const clearInlineMark = (
    ...
    return normalizeDocument({
        blocks: normalizedDocument.blocks.map((block, blockIndex) => {
            ...
            return transformBlockRange(block, overlapStart - blockStart, overlapEnd - blockStart, mark, 'remove');
        }),
    });
};
...
export const getSelectionMarks = (
    ...
    if (isSelectionCollapsed(normalizedSelection)) {
        return getMarksAtOffset(documentModel, selectionPointToLinearOffset(documentModel, normalizedSelection.anchor));
    }
